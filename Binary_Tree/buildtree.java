...
class Solution {
    public TreeNode build(int [] preorder,int preStart,int preEnd,int[] inorder,int inStart,int inEnd,Map<Integer,Integer> inMap)
    {
        ...
        root.left = build(preorder,preStart+1,preStart+numsLeft,inorder,inStart,x-1,inMap);
        root.right = build(preorder,preStart+numsLeft+1,preEnd,inorder,x+1,inEnd,inMap);
        return root;
    }
